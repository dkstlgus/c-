...
#define FALL 0	//비밀번호가 다를 때의 반환 값 0을 매크로 상수로 정의
...
int main()
{
	...
	do
	{
		display_menu();		//사용자에세 제공하는 서비스 목록 표시 
		printf("원하는 서비스를 선택하세요.");
		scanf("%d",&service);	//사용지기 원하는 서비스 번호 입력받기
		
		switch(service)
		{
			case 1: check_balance(); break; //잔액 조회 
			case 2: deposit(); break; //입금 처리 
			case 3: withdraw(); break;	//출금처리 
			case 4: exit(0);	//프로그램 종료 
		}
		
		/*사용자가 서비스 결과 화면 을 확인한 후 아무키나 누르면 현재 화면을 지우고 새 화면에서 다시 시작*/
...
int check_PW()
{
	static int count= 0;	//이 함수에서 발생한 비밀번호를 비교해 일치 여부 반환하기 
	int system_PW = 1111;	//정해진 시스템 서비스 비밀번호 
	int limit = 3;	//비밀번호 오류 제한 횟수 
	int input_PW;	//사용자가 압력한 비밀번호 
